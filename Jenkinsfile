                ...
                withEnv(["AWS=${AWS_HOST}", "PUBLIC=${PUBLIC_HOST}"]) {
                    withCredentials([string(credentialsId: 'CODEDXAPI', variable: 'token')]) {
                        sh '''
                          ...
                          curl -X \'PUT\' -k -H \'accept: application/json\' -H \'Content-Type: application/json\' -H "API-Key: $token" http://$AWS:8080/codedx/x/tool-connector-config/values/6 --data-raw '{\"host_url\":\"http://$AWSYUCK:8088\",\"auto-refresh-interval\":false,\"available-during-analysis\":true,\"access_token\":{\"remembered\":true},\"selected_project\":\"java-sec-code\"}'
